...
/* Prepare the Urls for usage...改变 * urls里面的内容，即要获取的内容
 */
	...
	for(state = 1; state <= 3; state++)
	{
		for(dev = devlist; dev; dev = dev->pNext)
		{
			...
			descXML = miniwget_getaddr(dev->descURL, &descXMLsize,
			   	                        lanaddr, lanaddrlen);      //miniwget2(url, hostname, port, path, size, addr, addrlen);
			if(descXML)
			{
				ndev++;
				...
				if(0==strcmp(data->servicetype_CIF,
				   ...
				   || state >= 3 )
				{
				  GetUPNPUrls(urls, data, dev->descURL);   //*urls即为要获取的controlURL信息
